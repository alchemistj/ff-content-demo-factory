...
test('incidental mold wording in website audit does not block a non-mold business', () => {
  const root = sandbox();
  const result = cp.runOne({ root, config: config(), candidate: {
    ...candidate('Electric Co'),
    category: 'Electrician',
    services: ['panel upgrades'],
    websiteAudit: { siteCopyEvidence: ['Customer asked whether we remove mold; we do not.'] },
  } });
  assert.equal(result.code, 'CLAIMED');
  assert.equal(result.run.candidate.websiteAudit.siteCopyEvidence.length, 1);
});
