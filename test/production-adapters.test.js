...
function cursorDouble() {
  ...
  return {
    ...
    async runResearchRecord({ kind, jobId, input }) {
      ...
      return { receipt: { provider: 'cursor-sdk', jobId, status: 'completed', resolvedModel: 'grok-4.6', runId: 'run-page-1' }, result: { kind, pages: fullPages(), serviceCoverageLedger: { version: 'canonical-service-coverage-ledger-v1', prospectId: 'p1', placeId: 'p1', aliases: {}, services: [{ id: 'ev-charging', name: 'EV Charging', reviewIds: ['r1'], currentSitePageUrls: [] }, { id: 'panel-upgrade', name: 'Panel upgrade', reviewIds: ['r2'], currentSitePageUrls: [] }] }, comparison: { candidates: [{ id: 'ev-charging', name: 'EV Charging' }, { id: 'panel-upgrade', name: 'Panel upgrade' }] } } };
    },
  };
}
...
test('review judgment is authoritative, receipt-bound, and feeds validated evidence prescription', async () => {
  ...
  const prescription = await adapters.prescriber.prescribe({ finalist, inventory: { exactPlace: true, discoverySampleOnly: false, dateWindow: null, requestedLimit: 50, listingReviewCount: 2, reviews: [review, review2], classifications: { r1: judgment, r2: judgment2 } }, decision: { serviceCoverageLedger: { version: 'canonical-service-coverage-ledger-v1', prospectId: 'p1', placeId: 'p1', aliases: {}, services: [{ id: 'ev-charging', name: 'EV Charging', reviewIds: ['r1'], currentSitePageUrls: [] }, { id: 'panel-upgrade', name: 'Panel Upgrade', reviewIds: ['r2'], currentSitePageUrls: [] }] }, candidateServices: [{ id: 'ev-charging', name: 'EV Charging' }, { id: 'panel-upgrade', name: 'Panel Upgrade' }], pages: fullPages(), whyBuilt: { text: 'The owned site shows an opportunity. A customer review documents completed EV charging work.', refs: [{ type: 'opportunity', id: 'opp-1' }, { type: 'review', id: 'r1' }] } } });
