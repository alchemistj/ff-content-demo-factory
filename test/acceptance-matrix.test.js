...
test('AC17 interruption resumes without repeating valid paid enrichment', async () => {
  ...
  const recovered = await runFactoryCycle({ root: h.root, config: h.config, adapters: { ...h.adapters, reviewJudge: { judge: async ({ review }) => { const service = review.id === 'r1' ? 'ev-charging' : 'panel-upgrade'; const judgment = { authoritative: true, decision: 'anchor', directCompletedService: true, service, serviceEvidence: [{ service, evidenceType: 'direct-completed-work' }], claims: [], model: 'cursor-grok-4.6-high', judgmentId: `recovery-${review.id}`, provenance: { reviewId: review.id, source: review.source || 'apify-finalist' } }; recoveryJudgments.push(judgment); return judgment; } } }, architectDecision: { qa: { passed: false, whyBuilt: { text: 'The clear site opportunity is buried. Direct review evidence supports the focused service direction.', refs: [{ type: 'opportunity', ref: 'clear site opportunity' }, { type: 'review', ref: 'r1' }] } } } });
