...
function adapters({ failJudge = false } = {}) {
  ...
  const proposal = {
    ...
    pages: [
      ...
      { type: 'Service', service: 'panel-upgrade', url: '/panel-upgrade', primaryKeyword: 'Panel upgrade Dallas', titleDirection: 'Panel Upgrade in Dallas', h1Direction: 'Panel upgrades grounded in real work', angle: 'Make the second direct service proof easy to find.', whyIncluded: 'Second evidence-backed service page.', overlapBoundaries: 'Do not duplicate EV charging.', claims: [], traps: [], strongestEvidence: 'r2', recommendedFirstReview: { reviewId: 'r2', reviewer: 'Anthony', rating: 5, date: '2026-01-02', exactText: reviews[1].text, why: 'Direct completed panel work.' } },
  ...
  return {
    ...
    prescriber: { propose: async ({ classification, prospectId, placeId, runId }) => { calls.proposal++; assert.equal(classification.authoritativeJudgmentCount, 2); proposal.serviceCoverageLedger = { ...proposal.serviceCoverageLedger, prospectId, placeId, runId }; return { proposal, receipt: { provider: 'cursor', status: 'completed', jobId: 'p1' } }; } },
