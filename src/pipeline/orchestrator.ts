...
export interface PipelineInput { handoff?: ApprovedProspectHandoff; prospectId?: string; prospect?: JsonObject; prescription?: JsonObject | null; evidence?: JsonObject | null; reviewInventory?: any; stateStore?: StateStore; writers?: WriterAdapters; qa?: QaAdapters; qaAdapters?: QaAdapters; wholeSiteAssessor?: Adapter; wholeSiteQa?: Adapter; wholeSiteRepair?: Adapter; guideProvider?: GuideProvider; guidesProvider?: GuideProvider; now?: Date | string | number; clock?: () => Date; maxRepairs?: number }
...
export function contextFor(state: PipelineState, stage: string, guides: LoadedGuides): Dict {
  const finishedBusinessCopy: Dict = { ...(state.outputs.servicePages || {}) }; for (const page of ['homepage', 'contact', 'header', 'footer']) if (state.outputs[page] !== undefined) finishedBusinessCopy[page] = state.outputs[page];
...
export async function runOneProspect(input: PipelineInput = {}): Promise<PipelineResult> {
  ...
  const qa = input.qa || input.qaAdapters; const provider = input.guideProvider || input.guidesProvider; const save = async (): Promise<void> => { state!.updatedAt = clock().toISOString(); await writeState(stateStore, state!); }; const services = prescribedServices(state.prescription);
  while (state.stage !== STAGES.COMPLETE) {
    if (state.stage === STAGES.WRITER_1) { const context = contextFor(state, 'writer1', await loadCanonicalGuides('writer1', provider)); state.stages[STAGES.WRITER_1] = { status: 'running' }; await save(); const result = await invoke(requireAdapter(input.writers, ['writer1', 'servicePages', 'services'], 'writer1'), { ...context, prescribedServicePages: services, idempotencyKey: `${state.runId}:writer1:write` }); const pages = normalizeExpectedPages(result, services, ['servicePages', 'pages'], 'Writer 1'); services.forEach((service) => assertPageIdentity(pages[service.pageId], service, 'service', `Writer 1 page ${service.pageId}`)); enforceReviewChoices('writer1', pages, services, state); state.outputs.servicePages = pages; state.stages[STAGES.WRITER_1] = { status: 'complete' }; markEvent(state, { type: 'stage-complete', stage: STAGES.WRITER_1 }, clock); state.stage = STAGES.QA_1; await save(); }
    ...
    else if (state.stage === STAGES.WRITER_2) { const context = contextFor(state, 'writer2', await loadCanonicalGuides('writer2', provider)); const destinations = state.prescription?.destinations || {}; const approvedSupportingPages = [{ pageId: 'homepage', ...(destinations.homepage || {}) }, { pageId: 'contact', ...(destinations.contact || {}) }, { pageId: 'header', routes: destinations.header || [] }, { pageId: 'footer', routes: destinations.footer || [] }]; state.stages[STAGES.WRITER_2] = { status: 'running' }; await save(); const result = await invoke(requireAdapter(input.writers, ['writer2', 'supportingPages', 'siteChrome'], 'writer2'), { ...context, finishedServicePages: state.outputs.servicePages, approvedSupportingPages, idempotencyKey: `${state.runId}:writer2:write` }); const pages = normalizeExpectedPages(result, approvedSupportingPages, ['pages', 'supportingPages', 'sitePages'], 'Writer 2'); assertPageIdentity(pages.homepage, destinations.homepage, 'homepage', 'Writer 2 homepage'); assertPageIdentity(pages.contact, destinations.contact, 'contact', 'Writer 2 contact'); assertApprovedChrome(pages, destinations); state.outputs = { ...state.outputs, ...pages }; state.stages[STAGES.WRITER_2] = { status: 'complete' }; state.stage = STAGES.QA_2; markEvent(state, { type: 'stage-complete', stage: STAGES.WRITER_2 }, clock); await save(); }
    ...
    else if (state.stage === STAGES.WRITER_3) { const context = contextFor(state, 'writer3', await loadCanonicalGuides('writer3', provider)); state.stages[STAGES.WRITER_3] = { status: 'running' }; await save(); const result = await invoke(requireAdapter(input.writers, ['writer3', 'strategyOverview', 'strategy'], 'writer3'), { ...context, finishedBusinessCopy: context.finishedBusinessCopy, idempotencyKey: `${state.runId}:writer3:write` }); const pages = normalizeExpectedPages(result, ['strategyOverview'], ['strategyOverview', 'pages'], 'Writer 3'); assertStrategyIdentity(pages.strategyOverview); state.outputs.strategyOverview = pages.strategyOverview; state.stages[STAGES.WRITER_3] = { status: 'complete' }; state.stage = STAGES.QA_3; markEvent(state, { type: 'stage-complete', stage: STAGES.WRITER_3 }, clock); await save(); }
