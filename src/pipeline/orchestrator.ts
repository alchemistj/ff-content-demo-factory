...
import { assertApprovedProspectHandoff, buildIntentLedger, digestOf, type ApprovedProspectHandoff } from '../contracts/index.js';
...
import { STAGES, createInitialState, createMemoryStateStore, handoffFingerprint, readState, reviewFingerprint, stageInputProjection, stagePrompt, type JsonObject, type PipelineState, type StateStore, validateState, writeState } from './state.js';
...
async function executeWriter(input: PipelineInput, production: boolean, stage: 'writer1' | 'writer2' | 'writer3', payload: Dict, adapter?: Adapter): Promise<{ output: any; receipt?: CursorWriterReceipt; threadUrl?: string; claim?: JsonObject }> {
  if (production) {
    ...
    try {
      const dispatched = await input.cursorWriter.dispatch(stage, payload, stagePrompt(stage), input.handoff?.sourceCheckpoint.runId || 'unknown-run');
      return { output: dispatched.output, receipt: dispatched.receipt, threadUrl: dispatched.threadUrl, ...(dispatched.claim ? { claim: dispatched.claim as JsonObject } : {}) };
    } catch (error) {
...
function bindWriterCompletion(state: PipelineState, stage: 'writer1' | 'writer2' | 'writer3', execution: { receipt?: CursorWriterReceipt; claim?: JsonObject }): void {
  if (!execution.receipt) return;
  state.writerReceipts[stage] = execution.receipt as any;
  if (execution.claim) state.writerClaims[stage] = execution.claim;
  const claim = execution.claim || {};
  state.writerBindings[stage] = { stage, stageProjectionDigest: digestOf(stageInputProjection(state, stage)), receiptDigest: digestOf(execution.receipt), inputDigest: execution.receipt.inputDigest, promptDigest: execution.receipt.promptDigest, outputDigest: execution.receipt.outputDigest, agentId: execution.receipt.agentId, jobId: execution.receipt.jobId, threadUrl: execution.receipt.threadUrl, claimKey: claim.key, ownerToken: claim.ownerToken };
}
...
export async function runOneProspect(input: PipelineInput = {}): Promise<PipelineResult> {
  ...
  if (!state) {
    ...
    try { assertApprovedProspectHandoff(input.handoff, { requireTrustedSource: input.production === true }); } catch (error) { throw new PipelineError('HANDOFF_INVALID', 'Approved prospect handoff failed contract validation', error); }
  ...
  while (state.stage !== STAGES.COMPLETE) {
    if (state.stage === STAGES.WRITER_1) { const context = contextFor(state, 'writer1', await loadCanonicalGuides('writer1', provider)); const payload = { ...context, prescribedServicePages: services, idempotencyKey: `${state.runId}:writer1:write` }; state.stages[STAGES.WRITER_1] = { status: 'running' }; await save(); const execution = await executeWriter(input, production, 'writer1', payload, production ? undefined : requireAdapter(input.writers, ['writer1', 'servicePages', 'services'], 'writer1')); const pages = normalizeExpectedPages(execution.output, services, ['servicePages', 'pages'], 'Writer 1'); services.forEach((service) => assertPageIdentity(pages[service.pageId], service, 'service', `Writer 1 page ${service.pageId}`)); enforceReviewChoices('writer1', pages, services, state); state.outputs.servicePages = pages; bindWriterCompletion(state, 'writer1', execution); state.stages[STAGES.WRITER_1] = { status: 'complete', ...(execution.receipt ? { receipt: execution.receipt } : {}) }; markEvent(state, { type: 'stage-complete', stage: STAGES.WRITER_1, ...(execution.threadUrl ? { threadUrl: execution.threadUrl } : {}) }, clock); state.stage = STAGES.QA_1; await save(); }
      ...
      state.outputs.humanGate2 = createHumanGate2Artifact({ websiteWords: assessment.site, reviewAnalysisFacts: state.handoff?.reviewAnalysisFacts, rejectedRoutes, rejectedIntentLedger: buildIntentLedger((state.handoff?.serviceComparison || []) as any) });
