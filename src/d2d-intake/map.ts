...
export function mapAdvancedBusinessToSeed(
  ...
  const correlation: WorkflowSourceCorrelation = {
    ...
    factoryQualificationReason: "Advanced by Content Factory from raw source facts.",
...
export function factoryProspectId(record: NormalizedRawBusiness): string {
  const raw = record.placeId ?? record.d2dBusinessId;
  const slug = raw
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "")
    .slice(0, 64);
  return slug.length > 0 ? slug : "place-unknown";
}
...
export function composeRawSourceNotes(
  ...
  const campaignBits = [
    ...
    envelope.campaign?.search?.query ? `campaign.search=${envelope.campaign.search.query}` : null,
  ].filter(Boolean);
  return [
    ...
    `exportedAt=${envelope.exportedAt};`,
    `${refs.join("; ")}.`,
    campaignBits.length ? campaignBits.join("; ") + "." : "",
  ]
    .filter(Boolean)
    .join(" ");
}
