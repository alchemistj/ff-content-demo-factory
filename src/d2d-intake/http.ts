...
async function handleGetBusiness(sourceBusinessId: string, deps: D2dIntakeHttpDeps): Promise<Response> {
  const receipt = await deps.registry.getReceiptByBusiness(sourceBusinessId);
