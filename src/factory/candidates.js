...
/** Join discovery rows and their audit results without using null as a key. */
function stableCandidateIdentity(candidate) {
  if (candidate?.placeId) return String(candidate.placeId);
  const mapsUrl = candidate?.mapsUrl || candidate?.googleMapsUrl;
  if (mapsUrl) return String(mapsUrl);
  return identityKey(candidate);
}
...
function buildCandidateBench(rawCandidates, websiteAudits = new Map(), options = {}) {
  ...
  return marked.map((candidate) => {
    const audit = auditShape(websiteAudits.get(stableCandidateIdentity(candidate)) || websiteAudits.get(candidate.placeId) || websiteAudits.get(candidate.website) || {});
...
module.exports = { MAX_DISCOVERY_CANDIDATES, normalizedWebsite, isMoldExcluded, identityKey, stableCandidateIdentity, deduplicateCandidates, buildCandidateBench, architectSelect };
