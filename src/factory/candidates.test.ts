...
test("website/opportunity evidence is required for factory selection", () => {
  ...
  const evidence = auditListingOpportunity(northline.record, NORTHLINE_SEARCH_CONTEXT);
...
test("seedable weak-opportunity garage-door listing is not selected", () => {
  ...
  assert.equal(canFormProspectSeed(weak.record, NORTHLINE_SEARCH_CONTEXT).ok, true);
  const evidence = auditListingOpportunity(weak.record, NORTHLINE_SEARCH_CONTEXT);
  const entry = buildCandidateEntry({
    record: weak.record,
    searchContext: NORTHLINE_SEARCH_CONTEXT,
    websiteEvidence: evidence,
...
test("seedable HVAC listing is not selected against garage-door search", () => {
  ...
  assert.equal(canFormProspectSeed(hvac.record, NORTHLINE_SEARCH_CONTEXT).ok, true);
  const evidence = auditListingOpportunity(hvac.record, NORTHLINE_SEARCH_CONTEXT);
  const entry = buildCandidateEntry({
    record: hvac.record,
    searchContext: NORTHLINE_SEARCH_CONTEXT,
    websiteEvidence: evidence,
