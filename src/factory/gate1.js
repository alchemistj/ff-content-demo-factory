...
function validatePolicyIntegrity(prescription) {
  ...
  try {
    const result = validatePagePolicy({ pages: prescription.pages, services: prescription.valueHierarchy, serviceLedger: prescription.serviceCoverageLedger || null, policy: prescription.pagePolicy, override: prescription.expansionOverride || null, runContext: { prospectId: prescription.prospect?.prospectId || prescription.prospect?.placeId, runId: prescription.runId || prescription.prospect?.runId, now: new Date().toISOString() }, sourceBinding: { sourceArtifactDigest: prescription.sourceArtifactDigest, serviceLedger: prescription.serviceCoverageLedger || null }, evidenceDigest: prescription.evidenceDigest || null });
