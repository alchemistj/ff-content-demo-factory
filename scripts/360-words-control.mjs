...
export function validateControl(control, input = {}) {
  ...
  if (recovery.recoveryVersion === "words-writer1-artifact-recovery/v3") {
    ...
    if (control.policy?.mode !== "validation-only" || Object.entries(EXPECTED_RECOVERY_V3_FINALIZE).some(([key, value]) => recovery[key] !== value) || recovery.promptDigest !== v3PromptDigest || recovery.priorRecoveryV3PromptDigest !== v3PromptDigest || typeof recovery.idempotencyKey !== "string" || !/^[^:\s]+:writer1:artifact-recovery:v3-finalize:sha256:[0-9a-f]{64}:sha256:[0-9a-f]{64}$/u.test(recovery.idempotencyKey) || !recovery.idempotencyKey.endsWith(`:${v3PromptDigest}`) || recovery.allowFollowUp !== undefined || recovery.allowResume !== undefined || recovery.allowCreate !== undefined || recovery.send !== undefined || control.policy.allowFollowUp !== undefined || control.policy.allowResume !== undefined || control.policy.allowCreate !== undefined || control.policy.send !== undefined) throw new Error("active validation-only v3-finalize wake is missing exact history, frozen-copy, no-message, canonical-prompt, or idempotency pins");
