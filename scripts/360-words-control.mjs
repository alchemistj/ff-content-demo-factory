...
export const EXPECTED_VERIFIED_WRITER1_APPROVED_ARTIFACT = Object.freeze({ actionRunId: "32845845871", artifactId: 9562364448, artifactZipDigest: "sha256:147dad95aa6985a3991d7e12212921b18cbbd71a65d2a614f413444aaababade", artifactZipSize: 33290, outputPath: "outputs/writer1-output.json", outputFileDigest: "sha256:0a2d99e3cfc223a84584251272f0137f4f0aea0cd30e192ebd3b2c4103554602", outputFileSize: 22186, receiptPath: "runtime/writer1-correction-v3-receipt.json", receiptDigest: "sha256:39905f7e32859b7388a0fe468ea445fb8a55d2c0fd5bfeea17955fd4e931477a", receiptSize: 61202, statePath: "runtime/state.json", stateDigest: "sha256:2e4dd6a80c0e35708f36a6452a87e64cc9fb8925c07e5e36aa2d687ab072653a", stateSize: 1687 });
export const EXPECTED_VERIFIED_WRITER1_APPROVAL_SEAL = Object.freeze({ recoveryVersion: "verified-writer1-approval-seal/v1", sourceBranch: "architect/360-words-canary-verified", actionRunId: EXPECTED_VERIFIED_WRITER1_APPROVED_ARTIFACT.actionRunId, artifactId: EXPECTED_VERIFIED_WRITER1_APPROVED_ARTIFACT.artifactId, artifactZipDigest: EXPECTED_VERIFIED_WRITER1_APPROVED_ARTIFACT.artifactZipDigest, artifactZipSize: EXPECTED_VERIFIED_WRITER1_APPROVED_ARTIFACT.artifactZipSize });
export const VERIFIED_WRITER2_PROMPT = "Verified Writer2 stage. After an independently signed Architect Writer1 approval, write only the Home page at /, Contact at /contact, and the shared header/footer. Use the sealed Writer1 output as input. Return complete words-writer2-output/v1 JSON only. Do not write service pages, Strategy Overview, QA, or approval artifacts.";
export const VERIFIED_WRITER2_PROMPT_DIGEST = `sha256:${createHash("sha256").update(JSON.stringify(VERIFIED_WRITER2_PROMPT)).digest("hex")}`;
...
export function selectVerifiedWriter1Dispatch(control) {
  ...
  if (control?.policy?.mode === "writer1-approval-seal" && recovery?.recoveryVersion === EXPECTED_VERIFIED_WRITER1_APPROVAL_SEAL.recoveryVersion) return "verified-writer1-approval-seal";
  if (control?.policy?.mode === "writer2-write" && recovery?.recoveryVersion === "verified-writer2-write/v1") return "verified-writer2-write";
...
export function validateControl(control, input = {}) {
  ...
  if (control.requestedBy !== "architect" || (control.stage !== "writer1" && control.stage !== "writer2")) throw new Error("only Architect verified Writer1/Writer2 control is permitted");
  const writer2Control = control.stage === "writer2" && control.policy?.mode === "writer2-write";
  if ((!writer2Control && control.policy?.writer1Only !== true) || (writer2Control && control.policy?.writer2Only !== true) || control.policy?.provider !== "cursor-sdk" || control.policy?.model !== "cursor-grok-4.6-high" || control.policy?.fast !== false) throw new Error("immutable verified writer policy mismatch");
  ...
  if (control.policy?.mode !== "artifact-recovery" && control.policy?.mode !== "validation-only" && control.policy?.mode !== "validation-report-only" && control.policy?.mode !== "writer1-correction" && control.policy?.mode !== "writer1-retrieval-only" && control.policy?.mode !== "writer1-seal-only" && control.policy?.mode !== "writer1-approval-seal" && control.policy?.mode !== "writer2-write") throw new Error("active verified wake mode is unsupported");
  ...
  const isVerifiedApprovalSeal = control.policy?.mode === "writer1-approval-seal" && recovery?.recoveryVersion === EXPECTED_VERIFIED_WRITER1_APPROVAL_SEAL.recoveryVersion;
  const isVerifiedWriter2 = control.stage === "writer2" && control.policy?.mode === "writer2-write" && recovery?.recoveryVersion === "verified-writer2-write/v1";
  const isVerifiedLane = isVerifiedCorrection || isVerifiedSeal || isVerifiedRetrieval || isVerifiedApprovalSeal || isVerifiedWriter2;
  if (input.verifiedLane === true && !isVerifiedLane) throw new Error("isolated verified lane permits only its bounded verified Writer1 or Writer2 path");
  if (!recovery || (!isVerifiedLane && Object.entries(EXPECTED_RECOVERY).some(([key, value]) => recovery[key] !== value))) throw new Error("active wake is missing the exact prior/run/source tuple");
  ...
  if (recovery.recoveryVersion === EXPECTED_VERIFIED_WRITER1_APPROVAL_SEAL.recoveryVersion) {
    const qa = recovery.independentQaArtifacts;
    const qaValid = Array.isArray(qa) && qa.length === 2 && new Set(qa.map((item) => item?.role)).size === 2 && qa.every((item) => item && (item.role === "content" || item.role === "evidence") && item.path === `qa/architect/writer1-${item.role}.json` && item.decision === "PASS" && /^sha256:[0-9a-f]{64}$/u.test(String(item.digest)) && Number.isSafeInteger(item.size) && item.size > 0);
    const approval = recovery.approval;
    const approvalValid = approval && approval.path === "qa/architect/writer1-approval.json" && /^sha256:[0-9a-f]{64}$/u.test(String(approval.digest)) && Number.isSafeInteger(approval.size) && approval.size > 0;
    if (control.stage !== "writer1" || control.policy?.mode !== "writer1-approval-seal" || control.policy.writer2Only !== undefined || control.policy.allowCreate !== undefined || control.policy.allowResume !== undefined || control.policy.allowFollowUp !== undefined || control.policy.send !== undefined || Object.entries(EXPECTED_VERIFIED_WRITER1_APPROVAL_SEAL).some(([key, value]) => recovery[key] !== value) || recovery.allowCreate !== false || recovery.allowResume !== false || recovery.allowFollowUp !== false || recovery.maxFollowUps !== 0 || recovery.send !== undefined || recovery.resume !== undefined || recovery.create !== undefined || !qaValid || !approvalValid) throw new Error("active Writer1 approval-seal wake is missing exact successful artifact, Architect QA, or approval pins");
  } else if (recovery.recoveryVersion === "verified-writer2-write/v1") {
    if (control.stage !== "writer2" || control.policy?.mode !== "writer2-write" || control.policy.writer1Only !== undefined || control.policy.allowCreate !== undefined || control.policy.allowResume !== undefined || control.policy.allowFollowUp !== undefined || control.policy.send !== undefined || recovery.sourceBranch !== "architect/360-words-canary-verified" || typeof recovery.runId !== "string" || !/^run-[A-Za-z0-9-]+$/u.test(recovery.runId) || recovery.requestedModel !== "cursor-grok-4.6-high" || recovery.resolvedModel !== "grok-4.6" || recovery.effort !== "high" || recovery.fast !== false || typeof recovery.inputDigest !== "string" || !/^sha256:[0-9a-f]{64}$/u.test(recovery.inputDigest) || recovery.promptDigest !== VERIFIED_WRITER2_PROMPT_DIGEST || typeof recovery.idempotencyKey !== "string" || recovery.idempotencyKey !== `${recovery.runId}:writer2:${recovery.inputDigest}:${VERIFIED_WRITER2_PROMPT_DIGEST}` || recovery.allowCreate !== true || recovery.allowResume !== false || recovery.allowFollowUp !== false || recovery.maxFollowUps !== 0 || recovery.create !== undefined || recovery.send !== undefined || recovery.resume !== undefined || !recovery.seal || recovery.seal.approvalPath !== "canary/runtime/architect-writer1-approval.json" || recovery.seal.receiptPath !== "canary/runtime/writer1-approved-receipt.json" || recovery.seal.outputPath !== "canary/outputs/writer1-output.json" || recovery.seal.artifactZipDigest !== EXPECTED_VERIFIED_WRITER1_APPROVED_ARTIFACT.artifactZipDigest || recovery.seal.artifactZipSize !== EXPECTED_VERIFIED_WRITER1_APPROVED_ARTIFACT.artifactZipSize || !/^sha256:[0-9a-f]{64}$/u.test(String(recovery.seal.approvalDigest)) || !Number.isSafeInteger(recovery.seal.approvalSize) || recovery.seal.approvalSize <= 0 || recovery.seal.writer1OutputDigest !== recovery.seal.writer1ReceiptOutputDigest) throw new Error("active Writer2 wake is missing exact sealed Writer1 approval, new-agent, model, or idempotency pins");
  } else if (recovery.recoveryVersion === EXPECTED_POST_DISPATCH_SEAL.recoveryVersion) {
  ...
  return { dormant: false, stage: control.stage, sourceSha: recovery.sourceSha };
}
