...
Write complete, evidence-supported, naturally useful local-service copy. Give each section a distinct homeowner job: direct answer, confirmed scope, process or selection expectations, a meaningful differentiator, the strongest adjacent proof, and a concise next step where supported. Synthesize related reviews into useful service guidance. Display each quotation once, in one form; never repeat the same quote as inline copy and a blockquote. Public copy must read like a strong local-service page, not an audit memo defending claims. Handle shop hours and job timelines once, naturally. Do not assert 24/7 coverage, weekend dispatch, same-day SLAs, warranties, framing or carpentry, or pricing guarantees.

Word count is diagnostic only and must never pass a page. Do not pad to reach 800 words or any other number, and do not cut useful evidence solely to fit a number. Generally write in the 650–900+ useful-body-word range where the sealed evidence supports it. If evidence is thin, do not manufacture generic copy. The former hard floor of at least 800 words each is revoked.
...
async function runArtifactRecoveryV3Finalize(root: string, control: Dict): Promise<{ status: string; stage: string; threadUrl?: string; recoveryRunId?: string }> {
  ...
  if (inspection.parsed !== undefined) {
    try {
      ...
      const normalizedJson = serializeWriter1OutputDeterministically(normalized.output);
      errors.push(...collectWriter1ValidationDiagnostics(normalizedJson, payload));
      if (errors.length === 0) parseAndValidateWriter1Output(normalizedJson, payload);
    } catch (error) {
